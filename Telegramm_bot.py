...
def del_allLmes(message):
    global button_id
    global amount3
    global photo_num_choice_id
    if amount3 > 0:
        bot.delete_message(message.chat.id, photo_num_choice_id)
        bot.delete_message(message.chat.id, button_id)
    amount3 += 1
def start_message(message):
    ...
    button_id = bot.send_message(message.chat.id, text="Выбери тематику фото", reply_markup=keyboard).message_id
